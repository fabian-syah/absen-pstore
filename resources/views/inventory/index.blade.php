            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                        ...
                        @if(in_array(strtolower(auth()->user()->role), ['admin', 'audit', 'leader', 'security', 'user_biasa', 'admin_gaji']))
                        ...
                        <div class="btn-group" role="group">
                            ...
                            {{-- Tombol Gudang --}}
                            <a href="{{ route('inventory.available') }}"
                                class="btn btn-sm {{ request()->routeIs('inventory.available') ? 'btn-success' : 'btn-outline-success' }}">
                        ...
                        <form action="{{ url()->current() }}" method="GET" class="d-flex">
                            <div class="input-group input-group-sm" style="width: 250px;">
                                <input type="text" name="search" class="form-control" placeholder="Cari aset / user..."
                            ...
                            <tbody>
                                @forelse($inventories as $item)
                                    <tr>
                                                ...
                                                <div class="text-center">
                                                    @if($item->item_photo_path)
                                                        <img src="{{ asset('storage/' . $item->item_photo_path) }}"
                                                            ...
                                                            data-bs-img-title="Fisik: {{ $item->item_name }}">
                                                    @else
                                                        <div class="bg-secondary d-flex align-items-center justify-content-center text-white"
                                                            style="width: 45px; height: 45px; border-radius: 4px;"><i
                                                                class="mdi mdi-image-off"></i></div>
                                                    @endif
                                        ...
                                        <td>
                                            <div class="fw-bold">{{ $item->item_name }}</div><small
                                                class="text-muted">{{ $item->serial_number }}</small>
                                        </td>
                                        <td>{{ ucfirst($item->category) }}</td>
                                        <td>
                                            @if($item->user) <span class="fw-bold">{{ $item->user->name }}</span><br><small
                                                class="text-muted">{{ $item->user->branch->name ?? '-' }}</small>
                                            ...
                                            <div class="d-flex gap-1">
                                                ...
                                                @if(in_array(strtolower(auth()->user()->role), ['admin', 'admin_gaji']))
                                                    ...
                                                    <form action="{{ route('inventory.destroy', $item->id) }}" method="POST"
                                                        class="d-inline" onsubmit="return confirm('Hapus?')">@csrf
                                                        @method('DELETE')<button class="btn btn-inverse-danger btn-icon btn-sm"><i
                                                                class="mdi mdi-delete"></i></button></form>
                                                @endif
                                                ...
                                                @if($canReturn)
                                                    <button type="button" class="btn btn-inverse-primary btn-icon btn-sm"
                                                        title="Kembalikan" data-bs-toggle="modal" data-bs-target="#returnModal"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title" id="previewTitle">Foto</h5><button type="button" class="btn-close"
                        data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body text-center"><img id="previewImage" src="" alt="Preview" class="img-fluid rounded"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="receiver_name" class="form-control" required
                                placeholder="Contoh: Pak Budi (Security) / Bu Siti (HRD)">
