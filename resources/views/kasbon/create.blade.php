                        ...
                        <form action="{{ route('kasbon.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-4">
                                ...
                                @if (auth()->user()->role == 'admin')
                                    <select name="user_id" class="form-select form-select-lg mb-2">
                                        @foreach ($users as $u)
                                            <option value="{{ $u->id }}">
                                                {{-- Gunakan null coalescing operator (??) untuk jaga-jaga jika data kosong --}}
                                ...
                                @else
                                    <input type="text" class="form-control form-control-lg bg-light"
                                        value="{{ auth()->user()->name }} ({{ auth()->user()->division ?? 'Umum' }} - {{ auth()->user()->branch ?? 'Pusat' }})"
                                        readonly>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="input-group input-group-lg">
                                    ...
                                    <input type="text" name="amount" id="rupiah"
                                        class="form-control fw-bold text-primary" placeholder="0" required
                                        autocomplete="off">
                                </div>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="description" class="form-control" rows="3"
                                    placeholder="Contoh: Biaya berobat sakit gigi, service motor dinas, dll..." required></textarea>
                            </div>
                                ...
                                <div class="d-flex gap-4 mb-3">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="payment_method" id="methodCash"
                                            value="cash" checked onclick="toggleBank(false)">
                                        <label class="form-check-label" for="methodCash">Tunai (Cash)</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="payment_method"
                                            id="methodTransfer" value="transfer" onclick="toggleBank(true)">
