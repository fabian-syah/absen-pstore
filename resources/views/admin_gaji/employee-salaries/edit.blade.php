...
<style>
    ...
    .form-control:focus, .form-select:focus { border-color: #FF4747; box-shadow: 0 0 0 3px rgba(255, 71, 71, 0.1); }
    
    /* Input Rupiah Style */
    .input-group .form-control { border-left: none; }
    .input-group:focus-within .input-group-text { border-color: #FF4747; }
    ...
    input:checked + .slider { background-color: #FF4747; border-color: #FF4747; }
    ...
    .privilege-box.active { background: #fff1f1; border-color: #FF4747; }
...
<div class="row justify-content-center py-4">
    <div class="col-lg-9 col-md-11">
        ...
        {{-- Tombol Kembali --}}
        <div class="mb-4">
            <a href="{{ route('admin-gaji.employee-salaries.index', [
                'page' => request('page', 1), 
                'search' => request('search'), 
                'category' => request('category')
            ]) }}" class="text-decoration-none text-muted fw-bold small">
                <i class="mdi mdi-arrow-left me-1"></i> Kembali ke Daftar Master Gaji Non Karyawan
            </a>
        </div>
        ...
        <div class="card shadow-sm border-0 rounded-4">
            ...
            <div class="card-header-clean d-flex justify-content-between align-items-start">
                <div>
                    <h3 class="fw-bold text-dark mb-1">Setting Master Gaji Non Karyawan</h3>
                    <p class="text-muted mb-0">Atur komponen gaji khusus untuk user ini.</p>
                </div>
                <div class="text-end">
                    <h5 class="fw-bold text-danger mb-0">{{ $user->name }}</h5>
            ...
            <div class="card-body-clean">
                ...
                <form action="{{ route('admin-gaji.employee-salaries.update', $user->id) }}" method="POST" id="salaryForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-5">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="mb-4">
                                    ...
                                    <select name="category" id="category" class="form-select form-select-lg shadow-none">
                                        <option value="employee" {{ ($user->employeeSalary->category ?? '') == 'employee' ? 'selected' : '' }}>Tetap</option>
                                    ...
                                    <div class="form-text text-muted mt-2 small lh-sm" id="cat_desc">Pilih jenis kontrak untuk menentukan komponen gaji.</div>
                                </div>
                        ...
                        <div class="col-md-7">
                            ...
                            <div id="form_employee" class="salary-section">
                                ...
                                <div class="privilege-box d-flex align-items-center {{ ($user->employeeSalary->use_privilege_mode ?? 0) ? 'active' : '' }}" id="privilegeBox">
                                    ...
                                    <div class="ms-2">
                                        ...
                                        <small class="text-muted lh-1 d-block mt-1" style="font-size: 0.75rem;">Bebas potongan absensi harian otomatis.</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div id="form_freelance" class="salary-section" style="display: none;">
                                ...
                                <div class="mb-3">
                                    <label class="form-label">Rate Harian</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                            ...
                            <div class="mt-4 pt-3 border-top">
                                <label class="form-label small text-secondary mb-2">Catatan Tambahan</label>
                                <textarea name="notes" class="form-control" rows="3" placeholder="Catatan khusus untuk master gaji ini">{{ $user->employeeSalary->notes ?? '' }}</textarea>
                            </div>
                    ...
                    <div class="d-flex justify-content-end gap-3">
                        ...
                        <button type="submit" class="btn btn-danger btn-lg px-5 fw-bold shadow-sm text-white" style="border-radius: 10px;">
...
<div class="error-modal" id="errorModal">
    <div class="modal-header-error">
        ...
        <div class="modal-title-error">Validasi Gaji Tokoh</div>
    </div>
    <div class="modal-body-error" id="errorMessage">
        Ops! Gaji Pokok tidak boleh lebih dari Rp 6.000.000.
    </div>
        ...
        document.getElementById('salaryForm').addEventListener('submit', function(e) {
            ...
            if (basicSalaryInput && basicSalaryInput.offsetParent !== null) {
                ...
                if (basicSalaryValue > 6000000) {
                    e.preventDefault();
                    showErrorModal('Gaji Pokok tidak boleh lebih dari Rp 6.000.000.');
                    return false;
                }
            }
            rupiahInputs.forEach(input => {
