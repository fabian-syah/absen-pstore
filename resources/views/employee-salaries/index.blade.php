            ...
            <div class="col-12">
                ...
                <div class="header-section d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <p class="mb-0">Kelola komponen gaji pokok, tunjangan, dan kategori karyawan dengan mudah dan
                            efisien.</p>
                    </div>
                    <div>
                            ...
                            <div class="row g-3 align-items-end">
                                <div class="col-md-4">
                                    ...
                                    <div class="search-wrapper">
                                        <i class="mdi mdi-magnify position-absolute text-secondary"
                                            style="top: 50%; left: 15px; transform: translateY(-50%); font-size: 1.1rem;"></i>
                                ...
                                <div class="col-md-2">
                                    <button type="submit"
                                        class="btn btn-dark w-100 fw-bold py-2 d-flex align-items-center justify-content-center gap-2">
                ...
                <div class="card card-modern">
                    <div class="card-body p-0">
                                        ...
                                        <tr>
                                            <td class="ps-4">
                                                <div class="d-flex align-items-center gap-3">
                                                    <div class="avatar-wrapper">
                                                        ...
                                                        @else
                                                            <div class="avatar-img bg-gradient text-white d-flex align-items-center justify-content-center fw-bold"
                                                                style="background: linear-gradient(135deg, var(--primary) 0%, #6366f1 100%); font-size: 1.2rem;">
                                                                {{ substr($user->name, 0, 1) }}</div>
                                                        @endif
                                                    </div>
                                                    <div class="user-info">
                                                        <h6 class="mb-1">{{ $user->name }}</h6>
                                                        <small class="text-primary fw-bold">ID:
                                                            {{ $user->login_id ?? '-' }}</small>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td>
                                                <div class="location-info">
                                                    <span><i
                                                            class="mdi mdi-sitemap text-secondary me-2"></i><strong>{{ $user->division->name ?? 'Non-Div' }}</strong></span>
                                                    <span><i
                                                            class="mdi mdi-map-marker text-secondary me-2"></i>{{ $user->branch->name ?? '-' }}</span>
                                                </div>
                                            </td>
                                                ...
                                                @if ($user->employeeSalary)
                                                    @if ($user->employeeSalary->category == 'freelance')
                                                        <div class="salary-info">
                                                            <h6>Rp
                                                                {{ number_format($user->employeeSalary->daily_salary, 0, ',', '.') }}
                                                            </h6>
                                                    ...
                                                    @elseif($user->employeeSalary->category == 'promotor')
                                                        <div class="salary-info">
                                                            <h6>Rp
                                                                {{ number_format($user->employeeSalary->promotor_bonus, 0, ',', '.') }}
                                                            </h6>
                                                    ...
                                                    @else
                                                        <div class="salary-info">
                                                            <h6>Rp
                                                                {{ number_format($user->employeeSalary->basic_salary, 0, ',', '.') }}
                                                            </h6>
                                                            <small>/ bulan</small>
                                                        </div>
                                                    @endif
                                            ...
                                            <td>
                                                @if ($user->employeeSalary && $user->employeeSalary->category == 'employee')
                                                    @php
                                                        $totalMaster =
                                                            $user->employeeSalary->basic_salary +
                                                            $user->employeeSalary->position_allowance +
                                                            $user->employeeSalary->owner_privilege;
                                                    @endphp
                                                    <div class="salary-info">
                                                        <h6 class="text-primary">Rp
                                                            {{ number_format($totalMaster, 0, ',', '.') }}</h6>
                                            ...
                                            <td class="text-center pe-4">
                                                <a href="{{ route('employee-salaries.edit', ['employee_salary' => $user->id, 'page' => request('page'), 'search' => request('search'), 'branch_id' => request('branch_id'), 'category' => request('category')]) }}"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="text-center py-5">
                                                <div class="mb-3">
                                                    <i class="mdi mdi-folder-open text-muted"
                                                        style="font-size: 3rem;"></i>
                                                </div>
                                                <h5 class="text-muted fw-bold">Tidak ada data ditemukan</h5>
                                                <p class="text-secondary">Coba ubah filter atau lakukan pencarian kembali
                                                </p>
                                            </td>
                                        </tr>
                                    @endforelse
                        ...
                        <div class="pagination-section">
                            <span>Halaman <strong>{{ $users->currentPage() }}</strong> dari
                                <strong>{{ $users->lastPage() }}</strong></span>
