...
<style>
    ...
    /* Typography Fix (Anti-Nyaru) */
    .text-primary-dark { color: #3f3e91; } /* Warna PStore lebih gelap */
    .fw-800 { font-weight: 800; }
    .text-dark-contrast { color: #111 !important; } /* Hitam Pekat */
    ...
    .nav-tabs-custom .nav-link:hover { color: #4B49AC; }
</style>
            ...
            <div class="card-body">
                <form action="{{ route('employee-salaries.index') }}" method="GET">
                    ...
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-md-2">
                            <button type="submit" class="btn btn-dark w-100 fw-bold py-2" style="border-radius: 10px;">
                                Terapkan
                            </button>
                        </div>
                    </div>
                        ...
                        <li class="nav-item">
                            <a class="nav-link {{ request('category') == 'unset' ? 'active' : '' }}" 
                               href="{{ route('employee-salaries.index', array_merge(request()->except(['category', 'page']), ['category' => 'unset'])) }}">
                                Belum Diatur <span class="badge bg-danger rounded-circle ms-1">{{ $users->whereNull('employeeSalary')->count() > 0 ? '!' : '' }}</span>
                            </a>
                        </li>
                    </ul>
                </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="ps-4">
                                    <div class="d-flex align-items-center">
                                        <div class="avatar-wrapper me-3">
                                            ...
                                            @else
                                                <div class="avatar-img bg-secondary d-flex align-items-center justify-content-center text-white fw-bold" style="font-size: 1.2rem;">
                                                    {{ substr($user->name, 0, 1) }}
                                                </div>
                                            @endif
                                        </div>
                                        <div>
                                            {{-- NAMA WARNA HITAM PEKAT --}}
                                ...
                                <td>
                                    <div class="d-flex flex-column">
                                        <span class="fw-bold text-dark" style="font-size: 0.85rem;">
                                            <i class="mdi mdi-sitemap me-1 text-muted"></i> {{ $user->division->name ?? 'Non-Div' }}
                                        </span>
                                        <span class="text-secondary small mt-1">
                                            <i class="mdi mdi-map-marker me-1"></i> {{ $user->branch->name ?? '-' }}
                                        </span>
                                    </div>
                                </td>
                                ...
                                <td>
                                    @if($user->employeeSalary)
                                        @if($user->employeeSalary->category == 'freelance')
                                            <h6 class="mb-0 fw-800 text-dark-contrast">
                                                Rp {{ number_format($user->employeeSalary->daily_salary, 0, ',', '.') }}
                                            </h6>
                                        ...
                                        @if($user->employeeSalary->category == 'employee')
                                            <div class="d-flex flex-column gap-1">
                                                <small class="text-dark fw-bold">
                                                    Jabatan: <span class="text-secondary fw-normal">Rp {{ number_format($user->employeeSalary->position_allowance, 0, ',', '.') }}</span>
                                                </small>
                                                <small class="text-dark fw-bold">
                                                    Privilege: <span class="text-secondary fw-normal">Rp {{ number_format($user->employeeSalary->owner_privilege, 0, ',', '.') }}</span>
                                                </small>
                                            </div>
                                        @elseif($user->employeeSalary->category == 'promotor')
                                            <small class="text-success fw-bold">
                                                <i class="mdi mdi-star"></i> Bonus: Rp {{ number_format($user->employeeSalary->promotor_bonus, 0, ',', '.') }}
                                            </small>
                                        @else
                                ...
                                <td class="text-center pe-4">
                                    <a href="{{ route('employee-salaries.edit', $user->id) }}" 
                                       class="btn btn-outline-primary btn-sm fw-bold px-3 py-1" 
