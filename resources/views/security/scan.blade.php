...
<body>
    ...
    <div class="result-overlay" id="resultOverlay">
        <div class="p-4 w-100" style="max-width: 500px;">
            ...
            <div class="result-profile-card" id="resultProfileCard">
                ...
                <div id="resultStatusBadge" class="mt-3"></div>
    ...
    <script>
        ...
        let currentUserData = null; // Variable untuk menyimpan data user
            ...
            fetch("{{ route('security.check-user') }}", {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        currentUserData = data.data; // Simpan data user
        ...
        function submitAttendance(type) {
            ...
            fetch("{{ route('security.store-attendance') }}", {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        // Kirim data user juga ke showResult
        ...
        function showResult(status, message, responseData, attendanceType) {
            ...
            if (status === 'success') {
                ...
                if (currentUserData) {
                    ...
                    // Fallback to response data if currentUserData not available
                    resultUserPhoto.src = responseData.profile_photo;
                    resultUserName.innerText = responseData.name;
                    resultUserRole.innerText = responseData.role + ' - ' + responseData.division;
                    resultUserDivision.innerText = responseData.division;
                    resultUserBranch.innerText = responseData.branch;
                }
                ...
                // Set status badge if late or early checkout
                resultStatusBadge.innerHTML = '';
                if (responseData.is_late) {
                    resultStatusBadge.innerHTML = '<span class="badge bg-danger px-3 py-2 fw-bold"><i class="fas fa-clock me-1"></i> TERLAMBAT</span>';
                } else if (responseData.is_early_checkout) {
                    resultStatusBadge.innerHTML = '<span class="badge bg-warning text-dark px-3 py-2 fw-bold"><i class="fas fa-running me-1"></i> PULANG CEPAT</span>';
                } else if (attendanceType === 'masuk') {
                    resultStatusBadge.innerHTML = '<span class="badge bg-success px-3 py-2 fw-bold"><i class="fas fa-check-circle me-1"></i> TEPAT WAKTU</span>';
                } else {
                    resultStatusBadge.innerHTML = '<span class="badge bg-info px-3 py-2 fw-bold"><i class="fas fa-user-clock me-1"></i> ABSEN PULANG</span>';
                }
