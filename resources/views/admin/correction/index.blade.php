...
@section('content')
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-body py-3">
                    <form action="{{ route('admin.correction.index') }}" method="GET"
    ...
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Data Absensi Masuk (Kantor): {{ \Carbon\Carbon::parse($date)->format('d M Y') }}
                    </h4>
                    <div class="table-responsive">
                        <table class="table table-hover table-striped">
                            ...
                            <tbody>
                                @forelse($attendancesOffice as $item)
                                    <tr>
                                        <td>
                                        ...
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center gap-2">
                                                ...
                                                @if($item->check_out_time)
                                                    <form action="{{ route('admin.correction.reset-checkout', $item->id) }}"
                                                        method="POST"
                                                        onsubmit="return confirm('Yakin ingin mereset jam pulang {{ $item->user->name }}? User harus absen pulang lagi nanti.');">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit" class="btn btn-sm btn-warning text-dark"
                                                            title="Hapus jam pulang saja">
                                                ...
                                                <form action="{{ route('admin.correction.destroy', $item->id) }}" method="POST"
                                                    onsubmit="return confirm('HATI-HATI! Data absensi {{ $item->user->name }} akan dihapus PERMANEN. Lanjutkan?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-sm btn-danger text-white"
                                                        title="Hapus seluruh data hari ini">
    ...
    {{-- Tabel Data WFH / Dinas --}}
    <div class="row mt-4">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card border-info">
                <div class="card-body">
                    <h4 class="card-title text-info">Data Absensi WFH / Dinas:
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-striped">
                            ...
                            <tbody>
                                @forelse($attendancesWfh as $item)
                                    <tr>
                                        <td>
                                            <div class="d-flex flex-column">
                                                <span class="fw-bold">{{ $item->user->name ?? 'User Terhapus' }}</span>
                                        ...
                                        <td>
                                            <span
                                                class="badge badge-info text-dark">{{ ucwords($item->presence_status ?? 'wfh') }}</span>
                                        </td>
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center gap-2">
                                                {{-- TOMBOL: HAPUS TOTAL (Hanya unuk WFH) --}}
                                                <form action="{{ route('admin.correction.destroy', $item->id) }}" method="POST"
                                                    onsubmit="return confirm('HATI-HATI! Data absensi WFH {{ $item->user->name }} akan dihapus PERMANEN. Lanjutkan?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-sm btn-danger text-white"
                                                        title="Hapus seluruh data hari ini">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-5 text-muted">
                                            <i class="mdi mdi-laptop-chromebook" style="font-size: 2rem;"></i><br>
                                            Tidak ada data absen WFH/Dinas pada tanggal ini.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
