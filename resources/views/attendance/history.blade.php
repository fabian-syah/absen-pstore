...
@section('content')
                    ...
                    <div class="row align-items-center justify-content-between g-3">
                        <div class="col-auto">
                            @php
                                ...
                                $prevRoute = isset($employee) ? route('team.branch.employee.history', array_merge(['branchId' => $employee->branch_id, 'employeeId' => $employee->id], $prevParams)) : route('attendance.history', $prevParams);
                            @endphp
                        ...
                        <div class="col-auto d-flex gap-2">
                            @php
                                ...
                                $nextRoute = isset($employee) ? route('team.branch.employee.history', array_merge(['branchId' => $employee->branch_id, 'employeeId' => $employee->id], $nextParams)) : route('attendance.history', $nextParams);
                            @endphp
    ...
    @if (isset($employee) && (auth()->user()->role == 'audit' || auth()->user()->role == 'admin'))
        @foreach ($history as $att)
            @if($att->id && is_numeric($att->id))
                {{-- Modal Verifikasi, Edit, dll bisa ditaruh disini sesuai kode asli Anda --}}
            @endif
        @endforeach
    @endif
@endsection
