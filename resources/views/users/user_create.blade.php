...
@section('content')
    ...
    <form class="forms-sample" action="{{ route('users.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <div class="col-md-6 grid-margin stretch-card">
                <div class="card shadow-sm border-0">
                    <div class="card-header-custom"><h5 class="mb-0 text-primary"><i class="mdi mdi-account-key me-2"></i>Data Akun & Pribadi</h5></div>
                    <div class="card-body">
                        <div class="mb-4">
                            <h6 class="text-muted text-uppercase small fw-bold mb-3">Informasi Dasar</h6>
                            <div class="form-group mb-3"><label class="form-label">Nama Lengkap <span class="text-danger">*</span> (Sesuai KTP)</label><input type="text" class="form-control" name="name" value="{{ old('name') }}" placeholder="Masukkan nama lengkap" required></div>
                            <div class="row">
                                <div class="col-md-6"><div class="form-group mb-3"><label class="form-label">Tanggal Lahir</label><input type="date" class="form-control" name="birth_date" value="{{ old('birth_date') }}"></div></div>
                                <div class="col-md-6"><div class="form-group mb-3"><label class="form-label">Tanggal Masuk (Join Date)</label><input type="date" class="form-control" name="hire_date" value="{{ old('hire_date') }}"></div></div>
                            </div>
                        </div>
                        <hr class="my-4">
                        <div class="mb-2">
                            <h6 class="text-muted text-uppercase small fw-bold mb-3">Kredensial Login</h6>
                            <div class="form-group mb-3"><label class="form-label">ID Login <span class="text-danger">*</span></label><div class="input-group"><span class="input-group-text bg-light"><i class="mdi mdi-account"></i></span><input type="text" class="form-control" name="login_id" value="{{ old('login_id') }}" placeholder="Username unik" required></div></div>
                            <div class="row">
                                <div class="col-md-6"><div class="form-group mb-3"><label class="form-label">Password <span class="text-danger">*</span></label><input type="password" class="form-control" name="password" placeholder="********" required></div></div>
                                <div class="col-md-6"><div class="form-group mb-3"><label class="form-label">Konfirmasi Password <span class="text-danger">*</span></label><input type="password" class="form-control" name="password_confirmation" placeholder="********" required></div></div>
                            </div>
                            <div class="form-group mb-3"><label class="form-label">Role Pengguna <span class="text-danger">*</span></label><select class="form-select" id="role" name="role" onchange="toggleInputs()" required><option value="">-- Pilih Role --</option>@foreach ($allowedRoles as $role)<option value="{{ $role }}" {{ old('role') == $role ? 'selected' : '' }}>{{ strtoupper(str_replace('_', ' ', $role)) }}</option>@endforeach</select></div>
                            ...
                            <div class="mt-4">
                                ...
                                <div class="security-access-card p-3 d-flex align-items-center justify-content-between unlocked" id="securityCard">
                                    <div class="d-flex align-items-center">
                                        <div class="security-icon-wrapper me-3"><i class="mdi mdi-cellphone-check" id="securityIcon"></i></div>
                                        <div>
                                            <div class="security-status-text text-success" id="securityStatusTitle">ABSENSI MANDIRI AKTIF</div>
                                            <small class="text-muted lh-1 d-block mt-1" id="securityStatusDesc">User bisa melakukan absen mandiri via HP (Selfie/Lokasi).</small>
                                        </div>
                                    </div>
                                    <div class="form-check form-switch m-0">
                                        <input class="form-check-input" type="checkbox" id="only_security_scan" name="only_security_scan" value="1" {{ old('only_security_scan') ? 'checked' : '' }} onchange="toggleSecurityCard(this)">
                                    </div>
                                </div>

                                {{-- 2. AI FACE DETECTION --}}
                                <div class="ai-access-card p-3 d-flex align-items-center justify-content-between on" id="aiCard">
                                    <div class="d-flex align-items-center">
                                        <div class="ai-icon-wrapper me-3"><i class="mdi mdi-face-recognition" id="aiIcon"></i></div>
                                        <div>
                                            <div class="security-status-text text-primary" id="aiStatusTitle">AI FACE DETECT ON</div>
                                            <small class="text-muted lh-1 d-block mt-1" id="aiStatusDesc">Wajib scan wajah saat absen mandiri.</small>
                                        </div>
                                    </div>
                                    <div class="form-check form-switch m-0">
                                        {{-- Hidden Input untuk memastikan value 0 terkirim jika unchecked --}}
                                        <input type="hidden" name="use_face_recognition" value="0">
                                        {{-- Default CHECKED untuk user baru --}}
                                        <input class="form-check-input" type="checkbox" id="use_face_recognition" name="use_face_recognition" value="1" checked onchange="toggleAICard(this)">
                                    </div>
                                </div>
                            </div>
                ...
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-2">
                            ...
                            <div class="form-group mb-3"><label class="form-label">Email</label><div class="input-group"><span class="input-group-text bg-light">@</span><input type="email" class="form-control" name="email" placeholder="contoh@email.com" value="{{ old('email') }}"></div></div>
                            <div class="form-group mb-3"><label class="form-label">WhatsApp</label><div class="input-group"><span class="input-group-text bg-light"><i class="mdi mdi-whatsapp"></i></span><input type="text" class="form-control" name="whatsapp" placeholder="08xxx" value="{{ old('whatsapp') }}"></div></div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card shadow-sm border-0 border-top border-3" style="border-top-color: #009688 !important;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="alert alert-info border-0 rounded-3 mb-4 shadow-sm">
                            ...
                            <strong>PENTING:</strong> Jam yang Anda input di sini adalah <strong>WAKTU LOKAL CABANG</strong> user tersebut. <br>
                            Contoh: Jika User di Bali (WITA), input 08:00 artinya 08:00 WITA. Jika di Jakarta, artinya 08:00 WIB.
                        </div>

                        <div class="row">
                            <div class="col-md-6 mb-3"><label class="form-label fw-bold">Jam Masuk (Check In)</label><input type="time" class="form-control form-control-lg" name="check_in_start" value="{{ old('check_in_start') }}"><small class="text-muted">Batas awal mulai absen masuk (Waktu Lokal)</small></div>
                            <div class="col-md-6 mb-3"><label class="form-label fw-bold">Jam Pulang (Check Out)</label><input type="time" class="form-control form-control-lg" name="check_out_start" value="{{ old('check_out_start') }}"><small class="text-muted">Batas awal mulai absen pulang (Waktu Lokal)</small></div>
                        </div>
        ...
        <div class="row mt-4 mb-5">
            <div class="col-12 text-center"><a href="{{ route('users.index') }}" class="btn btn-light btn-lg me-3 px-4">Batal</a><button type="submit" class="btn btn-primary btn-lg px-5 shadow"><i class="mdi mdi-content-save me-2"></i>Simpan User Baru</button></div>
        </div>
    </form>
@endsection
            ...
            <div class="col-12">
                <div class="card shadow-sm border-0 border-top border-3" style="border-top-color: #009688 !important;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3"><label class="form-label fw-bold">Jam Masuk (Check In)</label><input type="time" class="form-control form-control-lg" name="check_in_start" value="{{ old('check_in_start', $user->check_in_start ? date('H:i', strtotime($user->check_in_start)) : '') }}"><small class="text-muted">Batas awal mulai absen masuk (Waktu Lokal)</small></div>
                            <div class="col-md-6 mb-3"><label class="form-label fw-bold">Jam Pulang (Check Out)</label><input type="time" class="form-control form-control-lg" name="check_out_start" value="{{ old('check_out_start', $user->check_out_start ? date('H:i', strtotime($user->check_out_start)) : '') }}"><small class="text-muted">Batas awal mulai absen pulang (Waktu Lokal)</small></div>
                        </div>
                        <div class="mt-2 text-end"><button type="button" onclick="clearWorkHours()" class="btn btn-outline-danger btn-sm"><i class="mdi mdi-eraser me-1"></i> Reset Jam ke Kosong</button></div>
                    </div>
                </div>
