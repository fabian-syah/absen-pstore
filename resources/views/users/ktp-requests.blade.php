                                    ...
                                    @foreach ($users as $user)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @if ($user->profile_photo_path)
                                                    <div class="modal fade" id="modalProfile{{ $user->id }}"
                                            ...
                                            {{-- =================================== --}}
                                            {{-- KOLOM 3: KTP LAMA (FIXED)           --}}
                                            {{-- =================================== --}}
                                            <td class="text-center">
                                                {{-- Cukup cek apakah path-nya ada di database, jangan pakai file_exists di blade --}}
                                                @if (!empty($user->ktp_photo_path))
                                                    <img src="{{ asset('storage/' . $user->ktp_photo_path) }}"
                                                        alt="Old KTP" class="rounded shadow-sm border"
                                                        style="width: 80px; height: 50px; cursor: pointer; object-fit: cover;"
                                                        data-bs-toggle="modal"
                                                        data-bs-target="#modalOldKtp{{ $user->id }}"
                                                        onerror="this.onerror=null; this.src='{{ asset('images/default-ktp.jpg') }}';">
                                                    {{-- ^ Tambahan: Jika file fisik hilang, otomatis ganti ke default --}}

                                                    {{-- MODAL KTP LAMA --}}
                                                    <div class="modal fade" id="modalOldKtp{{ $user->id }}"
                                                        tabindex="-1" aria-hidden="true">
                                                        <div class="modal-dialog modal-dialog-centered modal-lg">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title">KTP Lama: {{ $user->name }}
                                                                    </h5>
                                                ...
                                                @else
                                                    {{-- Tampilan jika di Database NULL --}}
                                                    <div class="d-inline-block p-1 border rounded bg-light">
                                                        <span class="text-muted small" style="font-size: 10px;">Tidak Ada
                                                            File</span>
                                                    </div>
                                                    <small class="d-block text-muted mt-1" style="font-size: 11px;">Belum
                                                        upload KTP</small>
                                                @endif
                                            </td>
                                                ...
                                                @if ($user->ktp_photo_temp_path && file_exists(storage_path('app/public/' . $user->ktp_photo_temp_path)))
                                                    <div class="position-relative d-inline-block">
                                                        <img src="{{ asset('storage/' . $user->ktp_photo_temp_path) }}"
                                                            alt="New KTP" class="rounded border border-success shadow-sm"
                                                            style="width: 80px; height: 50px; cursor: pointer; object-fit: cover;"
                                                            data-bs-toggle="modal"
                                                    ...
                                                    {{-- MODAL KTP BARU --}}
                                                    <div class="modal fade" id="modalNewKtp{{ $user->id }}"
                                                        tabindex="-1" aria-hidden="true">
                                                        <div class="modal-dialog modal-dialog-centered modal-lg">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title text-success">Calon KTP Baru:
                                                                        {{ $user->name }}</h5>
                                                                    <button type="button" class="btn-close"
                                                                        data-bs-dismiss="modal"></button>
                                                                </div>
                                                                ...
                                                                <div class="modal-footer justify-content-center">
                                                                    <small class="text-muted">Pastikan data terlihat jelas
                                                                        sebelum disetujui.</small>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                @else
                                                    <span class="text-danger small"><i class="mdi mdi-alert"></i> File
                                                        Error</span>
                                                @endif
                                            </td>
                                            ...
                                            <td>
                                                <div class="d-flex gap-2">
                                                    {{-- Tombol Approve --}}
                                                    <form action="{{ route('users.approve-ktp', $user->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit"
                                                            class="btn btn-success btn-sm text-white rounded-circle p-2"
                                                    ...
                                                    {{-- Tombol Reject --}}
                                                    <form action="{{ route('users.reject-ktp', $user->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit"
                                                            class="btn btn-danger btn-sm text-white rounded-circle p-2"
                                                            style="width: 35px; height: 35px; display: flex; align-items: center; justify-content: center;"
