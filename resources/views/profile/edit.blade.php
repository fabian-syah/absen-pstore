...
@section('content')
        ...
        <div class="col-md-4 grid-margin stretch-card">
            <div class="card">
                <div class="card-body text-center">
                        ...
                        <a href="#" data-bs-toggle="modal" data-bs-target="#profilePhotoModal">
                            @if ($user->profile_photo_path)
                                <img src="{{ asset('storage/' . $user->profile_photo_path) }}" alt="profile"
                                    class="img-lg rounded-circle shadow-sm"
                    ...
                    @if ($user->is_verified)
                        <div class="badge badge-primary px-3 py-2 mb-4"><i class="mdi mdi-check-decagram"></i> Akun
                            Terverifikasi</div>
                    @else
                    ...
                    <div class="mb-4">
                        @if (!$user->profile_photo_path)
                            <form action="{{ route('profile.photo.update') }}" method="POST" enctype="multipart/form-data">
                                @csrf @method('PUT')
                                <label for="profile_photo" class="btn btn-sm btn-primary w-100"><i
                                        class="mdi mdi-camera"></i> Upload Foto Profil</label>
                        ...
                        @else
                            ...
                            @else
                                <button type="button" class="btn btn-sm btn-inverse-warning w-100" data-bs-toggle="modal"
                    ...
                    @if (isset($achievements) && $achievements->count() > 0)
                        <div class="text-start mb-4">
                            <div class="d-flex align-items-center justify-content-between border-bottom pb-2 mb-3">
                                ...
                                <span class="badge bg-warning text-dark">{{ $achievements->flatten()->count() }}
                                    Piala</span>
                            </div>
                            ...
                            <div class="award-container" style="max-height: 300px; overflow-y: auto;">
                                @foreach ($achievements as $year => $items)
                                    ...
                                    @foreach ($items as $award)
                                        <div
                                            class="award-card rank-{{ $award->rank }} mb-2 p-2 rounded d-flex align-items-center position-relative overflow-hidden">
                                            <div class="glow-bg"></div>
                                            <div class="award-icon me-3">
                                                @if ($award->rank == 1)
                                                    <i class="mdi mdi-trophy text-white"
                                                        style="font-size: 24px; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.3));"></i>
                                                @elseif($award->rank == 2)
                                                    <i class="mdi mdi-medal text-white"
                                                        style="font-size: 24px; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.3));"></i>
                                                @else
                                                    <i class="mdi mdi-medal-outline text-white"
                                                        style="font-size: 24px; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.3));"></i>
                                                @endif
                                            </div>
                                            <div class="flex-grow-1 position-relative z-index-1 text-white">
                                                <h6 class="mb-0 fw-bold" style="font-size: 14px;">
                                                    Juara {{ $award->rank }}
                                                    <span class="opacity-75 font-weight-normal ms-1"
                                                        style="font-size: 11px;">(Bulan
                                                        {{ \Carbon\Carbon::create()->month($award->month)->translatedFormat('F') }})</span>
                                                </h6>
                                                <small class="opacity-75" style="font-size: 11px;">
                                                    <i
                                                        class="mdi mdi-check-circle-outline me-1"></i>{{ $award->total_attendance }}
                    ...
                    <div class="text-start bg-light p-3 rounded border mb-4">
                        ...
                        @else
                            <div class="mb-2">
                                <button type="button" class="btn btn-inverse-info btn-sm w-100" data-bs-toggle="modal"
                                    data-bs-target="#ktpModal">
                                    <i class="mdi mdi-eye"></i> Lihat KTP Saya
                                </button>
                            </div>
                            @if ($user->ktp_request_status == 'pending')
                                <div class="badge badge-warning w-100 py-2"><i class="mdi mdi-clock"></i> Menunggu
                                    Approval</div>
                            @elseif ($user->ktp_request_status == 'rejected')
                                ...
                                <button type="button" class="btn btn-outline-warning btn-sm w-100"
                                    data-bs-toggle="modal" data-bs-target="#changeKtpModal">
                    ...
                    <form class="forms-sample mt-4" action="{{ route('profile.update') }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <input type="date" class="form-control bg-light" name="birth_date"
                            ...
                            <div class="col-12 mb-4">
                                <div class="card bg-light border-0">
                                    <div class="card-body p-3 d-flex justify-content-between align-items-center">
                                        <div>
                                            <h6 class="fw-bold mb-1"><i class="mdi mdi-face-recognition me-1"></i>
                                                Aktifkan Deteksi Wajah (AI)</h6>
                                            <p class="text-muted small mb-0" style="line-height: 1.2;">
                                                Matikan fitur ini jika Anda menggunakan cadar (niqab) atau masker medis yang
                                                menutupi wajah saat absen.
                                                <br><span class="text-danger fw-bold">*Jika dimatikan, tombol kamera akan
                                                    langsung aktif tanpa scan wajah.</span>
                                            </p>
                                        </div>
                                        <div class="form-check form-switch ms-3">
                                            ...
                                            <input class="form-check-input" type="checkbox" id="useAI"
                                                name="use_face_recognition" value="1"
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <input type="text" class="form-control bg-light"
                                    value="{{ $user->branch->name ?? 'Pusat / Semua Cabang' }}" readonly>
                            </div>
                            <div class="col-md-6 mb-4">
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <input type="text" class="form-control bg-light"
                                    value="{{ $user->hire_date ? \Carbon\Carbon::parse($user->hire_date)->translatedFormat('d F Y') : '-' }}"
                                    readonly>
                            </div>
                            <div class="col-md-6 mb-4">
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" name="whatsapp"
                                        value="{{ old('whatsapp', $user->whatsapp) }}" placeholder="0812...">
                                </div>
                            </div>
                            <div class="col-md-6 mb-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" name="instagram"
                                        value="{{ old('instagram', $user->instagram) }}" placeholder="username">
                                </div>
                            </div>
                            <div class="col-md-6 mb-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" name="tiktok"
                                        value="{{ old('tiktok', $user->tiktok) }}" placeholder="username">
                                </div>
                            </div>
                            <div class="col-md-6 mb-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" name="facebook"
                                        value="{{ old('facebook', $user->facebook) }}" placeholder="username / link">
                                </div>
                            </div>
                            ...
                            <div class="col-12 mb-2 mt-2">
                                <div class="d-flex justify-content-between align-items-center border-bottom pb-2">
                                    ...
                                    <button type="button" class="btn btn-outline-dark btn-sm pt-0 pb-0"
                                        id="btn-toggle-password" style="font-size: 11px;">
                                ...
                                <div class="row p-3 mb-3 bg-light rounded border mx-1">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="password" class="form-control" name="password_confirmation"
                                            id="input-password-confirm" placeholder="Ulangi password">
                                    </div>
                                    <div class="col-12">
                                        <small class="text-danger">* Biarkan kosong jika tidak ingin mengganti
                                            password.</small>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="modal-body text-center">
                    @if ($user->profile_photo_path)
                        <img src="{{ asset('storage/' . $user->profile_photo_path) }}"
                            class="img-fluid rounded shadow-lg" style="max-height: 80vh;">
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Ganti Foto Profil</h5><button class="btn-close"
                        data-bs-dismiss="modal"></button>
                </div>
                <form action="{{ route('profile.photo.request') }}" method="POST" enctype="multipart/form-data">@csrf
                    <div class="modal-body"><input type="file" name="profile_photo" class="form-control" required>
                    </div>
    ...
    @if ($user->ktp_photo_path)
        <div class="modal fade" id="ktpModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-body text-center bg-dark"><img
                            src="{{ asset('storage/' . $user->ktp_photo_path) }}" class="img-fluid rounded"
    ...
    <div class="modal fade" id="changeKtpModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Ganti KTP</h5><button class="btn-close" data-bs-dismiss="modal"></button>
                </div>
